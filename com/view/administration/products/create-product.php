...
?>
	...
	<div style="margin-top: 80px">
		...
		if (isset($languages) == false || count($languages) == 0) {
        	?>
            <div id="success-alert" class="alert alert-success" role="alert">
            	Няма създадени езици, моля създайте език преди да създавате продукти.
            </div>
			<?php
		} else if (isset($categories) == false || count($categories) == 0) {
		    ?>
            <div id="success-alert" class="alert alert-success" role="alert">
            	Няма създадени категории, моля създайте категории преди да създавате продукти.
            </div>
			<?php
		} else {
		    ?>
                  	...
                  	<div class="form-group row">
                    	...
                    	<?php 
                    		foreach ($categories as $category) {
                		        if ($languages[0]->id() == $category->languageId()) {
                                	?>
                            			<option class="lg-<?php echo $category->languageId(); ?>-category-options" value="<?php echo $category->id(); ?>">
                            				<?php echo $category->name(); ?>
                        				</option>
                                	<?php
                		        } else {
                		            ?>
                            			<option class="lg-<?php echo $category->languageId(); ?>-category-options" value="<?php echo $category->id(); ?>" style="display: none">
                            				<?php echo $category->name(); ?>
                        				</option>
                                	<?php
                		        }
	...
	<script>
		...
		$("#language").on("change", function(oElement) {
			$.each($("#category")[0].options, function(iIndex, oElement) {
				oElement.style.display = "none";
			});

			$.each($(".lg-" + $("#language").val() + "-category-options"), function(iIndex, oElement) {
